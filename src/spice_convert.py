...
SAploy = np.array([16.907145985921161, 0.622223077618381, 0.000223330499353,
                    0.706643311920640, -0.000454964160674, 0.000312654210024,
                    -0.000103973707417, 0.000113069609374, 0.000026554807951,
                    -0.000044377349558])
...
def SA_poly_spiceness0(sigma0, spiciness0):

    SA = SAploy[0] \
       + sigma0 *(SAploy[1] + SAploy[5] *spiciness0 \
       + sigma0 *(SAploy[2] + SAploy[6] *spiciness0 + SAploy[8]*sigma0)) \
       + spiciness0 *(SAploy[3] + spiciness0 *(SAploy[4] + SAploy[7]*sigma0 + SAploy[9]*spiciness0))

    return SA

def deriv_SA_poly_spiciness0(sigma0,spiciness0):

    SA_poly_spiciness0_derivative = SAploy[3] \
                                  + sigma0 *(SAploy[5] + SAploy[6]*sigma0) \
                                  + spiciness0 *(2 *(SAploy[4] + SAploy[7]*sigma0) \
                                  + 3 *SAploy[9]*spiciness0)

    return SA_poly_spiciness0_derivative


def SA_CT_from_sigma0_spiciness0(sigma0,spiciness0):
    ...
    SA = SA_poly_spiceness0(sigma0, spiciness0)

    CT, _ = gsw.CT_from_rho(rho,SA,0) # The second solution is ignored.
    ...
    #--------------------------------------------------------------------------
    for _ in range(7):
        delta_spiciness = spiciness0 - gsw.spiciness0(SA,CT)
        derivative = deriv_SA_poly_spiciness0(sigma0, spiciness0)
        SA_old = SA
        SA = SA_old + delta_spiciness *derivative
        CT, _ = gsw.CT_from_rho(rho,SA,0) # The second solution is ignored.

    CT[(SA < 0) | (SA > 42)] = np.nan
    SA[(SA < 0) | (SA > 42)] = np.nan
    CT[(CT < -5) | (CT > 40)] = np.nan
    SA[(CT < -5) | (CT > 40)] = np.nan
