...
for i, c in enumerate(clrs):
    plt_i = ~m_i[i]
    plt_i[:30] = False
    stats = eng.field_stats(amps_dB[plt_i, i, :])
    ax.plot(stats['r_a'] / 1e3, stats['mean'], color=c)
    #ax.plot(stats['r_a'] / 1e3, stats['mean'] + stats['rms'], linewidth=0.5, color=c)
    #ax.plot(stats['r_a'] / 1e3, stats['mean'] - stats['rms'], linewidth=0.5, color=c)
    ax.plot(stats['r_a'] / 1e3, stats['10th'], linewidth=0.5, color=c)
    ax.plot(stats['r_a'] / 1e3, stats['90th'], linewidth=0.5, color=c)



for i, (e, c, m) in enumerate(zip(amps_dB[:, field_i, :], clrs, m_i[field_i, :])):
    #clr = clrs[0] if m | (i > 30) else clrs[-1]
    zorder, clr = (1, clrs[0]) if m | (i < 30) else (10, clrs[-1])
    ax.plot(r_a / 1e3, e, color=clr, alpha=0.8, zorder=zorder, linewidth=1)
