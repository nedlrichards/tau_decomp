...
dyn_1 = {'bg':[], 'tilt':[], 'spice':[], 'total':[]}
e_ri_1 = []

tl_files = list_tl_files(fc=1e3)
for tl in tl_files:
    eng_mode = MLEnergy(tl)
    eng_bg = eng_mode.background_diffraction('bg')
    e_ri_1.append(10 * np.log10(eng_bg * eng_mode.r_a))

    for ft in cf.field_types:

        proj_amp, proj_scale = eng_mode.proj_mode(ft, mode_num=2)
        proj_eng = np.abs(proj_amp) ** 2 / proj_scale
        dyn_1[ft].append(10 * np.log10(proj_eng))


eng_400 = EngProc(Config(fc=400, source_depth=source_depth), fields=['bg'])
e_ri_400 = eng_400.diffraction_bg()

dyn_400 = {'bg':[], 'tilt':[], 'spice':[], 'total':[]}
e_ri_400 = []

tl_files = list_tl_files(fc=400)
for tl in tl_files:
    eng_mode = MLEnergy(tl)
    eng_bg = eng_mode.background_diffraction('bg')
    e_ri_400.append(10 * np.log10(eng_bg * eng_mode.r_a))

    for ft in cf.field_types:

        proj_amp, proj_scale = eng_mode.proj_mode(ft, mode_num=1)
        proj_eng = np.abs(proj_amp) ** 2 / proj_scale
        dyn_400[ft].append(10 * np.log10(proj_eng))



lines_400 = {'bg':np.array(dyn_400['bg']) - e_ri_400,
             'tilt':np.array(dyn_400['tilt']) - e_ri_400,
             'spice':np.array(dyn_400['spice']) - e_ri_400,
             'total':np.array(dyn_400['total']) - e_ri_400}
lines_1 = {'bg':np.array(dyn_1['bg']) - e_ri_1,
           'tilt':np.array(dyn_1['tilt']) - e_ri_1,
           'spice':np.array(dyn_1['spice']) - e_ri_1,
           'total':np.array(dyn_1['total']) - e_ri_1}
...
def sparkline(ax, lines, stats, dy, title, ylim=(-20, 5), dx=0):
...
fig.savefig(join(savedir, f'shallow_coupling.png'), dpi=300)
