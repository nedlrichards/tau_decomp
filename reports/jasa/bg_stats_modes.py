...
for tl in tl_list:
    ml_pe = MLEnergyPE(tl)
    eng_pe.append(10 * np.log10(ml_pe.ml_energy('bg') * ml_pe.r_a))
...
ax[0].plot(r_a[diff_i] / 1e3, norm_eng[:, diff_i].T, '0.7', linewidth=0.5)
ax[1].plot(r_a[diff_i] / 1e3, eng_pe[:, diff_i].T, '0.7', linewidth=0.5)


fig, ax = plt.subplots()
ax.plot(r_a[diff_i] / 1e3,  norm_eng[:, diff_i].T - eng_pe[:, diff_i].T, '0.7', linewidth=0.5)
"""
