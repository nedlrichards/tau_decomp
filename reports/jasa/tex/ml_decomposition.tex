...
Blocking features also lead to a brief increase in TL energy as beams of energy leaving the ML pass through the TL. The horizontal distance this energy remains in the TL is calculated here by assuming the beam leaves the ML at grazing incidence angle at the SLD. The mean profile and SLD are used for this calculation here,
\begin{equation}
    k_x = \frac{2 \pi}{c_{SLD}},
\end{equation}
where $k_x$ is the modeled horiontal wavenumber of each beam and $c_{SLD}$ is the sound speed at the SLD. The horizontal distance travelled by this ray to a depth $z$ is
\begin{equation}
    (z - z_{SLD}) = \int^x_0 k_y dx,
\end{equation}}
where the vertical wavenumber $k_y^2 = k^2 + k_x^2$, where $k$ is the local acoustic wavenumber. A beam of energy leaving the ML at position $x$ then enters the TL at $x + \Delta x_0$ and exits at positon $x + \Delta x_1$. The contribution of the energy between these two positions is assumed to be superimposed, leading to a model of the TL eneergy as a moving sum filter of energy leaving the ML,
\begin{equation}
    E_{TL} = \int_{x-\Delta x_0}^{x+ \Delta x_1} \Delta E_{ML} dx,
    \label{eq:TL_eng}
\end{equation}
where $E_{ML}$ is the change in ML energy compensated for cylindrical spreading.

TODO: Calculation for $\Delta E_{ML}=-3$ dB over 5 km.
